...
def pearson_correlation(count, ips, curr_link, link_todelete, link, correlation_threshold):

    # correlation_threshold is the set value of the correlation from which the compensation algorithm is performed

    # Defining variables to work with
    trsl = link['trsl']
    temperature_tx = np.array(link['temperature_tx'])
    #temperature_tx = np.array(link['temperature_rx'])

    b_trsl_array = trsl[0]
    b_temptx_array = temperature_tx[0]
    #b_temptx_array = temperature_tx[1]

    a_trsl_array = trsl[1]
    a_temptx_array = temperature_tx[1]
    #a_temptx_array = temperature_tx[0]
    ...
    if not (np.isnan(pcctrsl_a) or np.isnan(pcctrsl_b)):
        if ((pcctrsl_a <= correlation_threshold)
            or (pcctrsl_a >= -correlation_threshold)) \
                or ((pcctrsl_b <= correlation_threshold) or (pcctrsl_b >= -correlation_threshold)):
